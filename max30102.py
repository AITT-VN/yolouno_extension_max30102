...
TIMETOBOOT = 3000 # // wait for this time(msec) to output SpO2
SCALE = 88.0 #//adjust to display heart beat and SpO2 in the same scale
...
SAMPLING = 5 #//if you want to see heart beat more precisely , set SAMPLING to 1
FINGER_ON = 5000 # // if red signal is lower than this , it indicates your finger is not on the sensor
...
class MAX30102(object):
    def __init__(self,
        ...
        # current calulcated data
        self.SpO2 = 95
        self.eSpO2 = 95 # raw SpO2 before low pass filtered
        self.Ebpm = 0 # estimated Heart Rate (bpm)
    ...
    async def scan(self):
        aveRed = 0 #;//DC component of RED signal
        aveIr = 0 #;//DC component of IR signal
        sumIrRMS = 0 #; //sum of IR square
        sumRedRMS = 0 #; // sum of RED square
        Num = 100 # calculate SpO2 by this sampling interval

        self.eSpO2 = 95.0 # initial value of estimated SpO2
        FSpO2 = 0.7 # filter factor for estimated SpO2
        frate = 0.95 # low pass filter for IR/red LED value to eliminate AC component
        ...
        while True:
            ...
            fred = 0.0
            fir = 0.0
            SpO2 = 0 # raw SpO2 before low pass filtered
            self.check()
            while self.available():
                ...
                if red > FINGER_ON:
                    if not beat and red > threshold_on:
                        beat = True
                    ...
                    if beat and red < threshold_off:
                        beat = False
                    if beats >= MIN_BPM and beats <= MAX_BPM:
                        #print('BPM: ', beats)
                        self.Ebpm = round(beats, 1)
                    else:
                        #print('calculating...')
                        pass
                else:
                ...
                if ir < FINGER_ON:
                    self.eSpO2 = MIN_SPO2 # //indicator for finger detached
                    self.SpO2 = MIN_SPO2
                ...
                if (loopCnt % Num) == 0:
                    ...
                    self.eSpO2 = round(FSpO2 * self.eSpO2 + (1.0 - FSpO2) * self.SpO2, 1) # low pass filter
                    #print('From class ** SpO2: ', self.SpO2, ", eSPO2: ", self.eSpO2, ', HR: ', self.Ebpm)
                    sumRedRMS = 0.0
                    sumIrRMS = 0.0
                    loopCnt = 0 # reset mean square at every interval
                    break
